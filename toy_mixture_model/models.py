...
class GenerativeNormalModel(AbstractGenerativeModel):
    ...
    def log_likelihood(self, x):
        mu, sigma = self.params["mu"], self.params["sigma"]
        return - ((x - mu)**2.0 / (2 * sigma ** 2.0)) - np.log(sigma)

    def predict_proba(self, x):
        likelihoods = np.array(list(map(self.log_likelihood, x)))
        return np.exp(likelihoods) / np.sqrt(2*np.pi)
    ...
    def d_log_likelihood(self, x):
        ...
        d_mu = x_normed / sigma
        d_sigma = (x - mu)**2.0 / (sigma ** 3.0) - 1.0 / sigma
...
class GenerativeNormalMixtureModel(AbstractGenerativeModel):
    ...
    def log_likelihood(self, x):
        ...
        log_likelihoods = - (x - mu) ** 2.0 / (2 * sigma**2.0)  - np.log(sigma)
    ...
    def predict_proba(self, x):
        return np.exp(self.log_likelihood(x)) / np.sqrt(2 * np.pi)
...
class GenerativeNormalQuasiMixtureModel(AbstractGenerativeModel):
    def __init__(self, mu, sigma, updates=None):
        if not updates:
            updates = ["mu", "sigma"]
        assert sigma > 0
        super(GenerativeNormalQuasiMixtureModel, self).__init__(["mu", "sigma"], np.array([float(mu), float(sigma)]), updates)
    ...
    def log_likelihood(self, x):
        mu, sigma = self.params["mu"], self.params["sigma"]
        return np.log(  np.exp( -((x - mu)**2) / (2 * sigma**2) ) + np.exp( -((x + mu)**2) / (2 * sigma**2) )  ) - np.log(sigma)

    def predict_proba(self, x):
        likelihoods = np.array(list(map(self.log_likelihood, x)))
        return np.exp(likelihoods) / (2 * np.sqrt(2*np.pi))

    def d_log_likelihood(self, x):
        mu, sigma = self.params["mu"], self.params["sigma"]
        denom = 1 / ( np.exp( -((x - mu)**2) / (2 * sigma**2) ) + np.exp( -((x + mu)**2) / (2 * sigma**2) ) )

        d_mu = denom * ( ((x - mu) / sigma**2) * np.exp( -((x - mu)**2) / (2 * sigma**2) ) - ( (x + mu)/sigma**2 ) * np.exp( -((x + mu)**2) / (2 * sigma**2) )  )
        d_sigma = denom * ( ((x - mu)**2 / sigma**3) * np.exp( -((x - mu)**2) / (2 * sigma**2) ) +  ((x + mu)**2 / sigma**3) * np.exp( -((x + mu)**2) / (2 * sigma**2) ) )  - 1/(sigma)
        return np.array([d_mu, d_sigma]).reshape((2, -1))
