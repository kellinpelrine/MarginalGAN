...
class AdversarialCompetition(object):
    def __init__(self, true_model, discriminative, generative,
                 gradient_descent, x_dataset, size_batch=100):
        self.l_auc = []
    ...
    def iteration(self):
        ...
        # np.random.seed(1)
        #dataset_seed = np.random.randint(1,60000)
        #np.random.seed(dataset_seed)
        x_true = np.random.choice(self.x_dataset,self.size_batch,replace=False).reshape(-1,1)
        #np.random.seed(len(self.generatives))
